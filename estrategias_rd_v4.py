# ====================================================================================
# DESAGREGACIÓN DE RIESGO -> ESTRATEGIAS  (V4, compacto, sin PNG en disco)
# ====================================================================================
import numpy as np, pandas as pd, joblib
from io import BytesIO
import matplotlib
matplotlib.use("Agg")                       # sin display, menor memoria
...
BINS_OBJETIVO = [5, 2]                       # genéricas: prueba 5, luego 2 bins
SALDO_MIN_BINS, SALDO_MAX_BINS = 2, 3        # variables de saldo: entre 2 y 3 bins
# puntuacion_cal_cat: se prueban estos dos esquemas de split (se usa el 1ro factible)
#   esquema 1: cada valor 1..6 en su propio bin
...
DIRECCION_NEGOCIO = {
    ...
    "segmentacion_gdp_v2": +1, "score_g": +1, "prob_malo": +1, "score": -1,
}

SUBJECT_COL = "key_value"
...
def optbinning_fit(x, y, nombre, direccion):
    ...
    - puntuacion_cal_cat: prueba los esquemas de USER_SPLITS_CAL.
    - variables de saldo: entre SALDO_MIN_BINS y SALDO_MAX_BINS.
    - resto: BINS_OBJETIVO."""
...
def main(df=None):
    ...
    construir_workbook(df, "A", f"arbol_solo_{TGT}_V4.xlsx")
    construir_workbook(df, "B", f"optbinning_arbol_{TGT}_V4.xlsx")
    print(f"Listo: arbol_solo_{TGT}_V4.xlsx y optbinning_arbol_{TGT}_V4.xlsx")

    excluir = [c for c in VARS_EXCLUIR if c in df.columns]
    if excluir:
        construir_workbook(df, "A", f"arbol_solo_{TGT}_sinalgunasvariables_V4.xlsx", excluir)
        construir_workbook(df, "B", f"optbinning_arbol_{TGT}_sinalgunasvariables_V4.xlsx", excluir)
