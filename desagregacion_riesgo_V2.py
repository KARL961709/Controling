# ====================================================================================
# DESAGREGACIÓN DE RIESGO — árbol monótono + optbinning (WoE)
# Versión modificada:
#   * Imputación: si una variable tiene >5% de missing, esos missing se reemplazan
#     por el WoE del bin de MAYOR riesgo (RD -> mayor RD promedio; score -> menor
#     score promedio). Primero se hace el WoE, luego la imputación, luego entra al árbol.
#   * Guarda el modelo (árboles por escenario + optbinning + ranking de estrategias)
#     en un .pkl para reaplicarlo a una base nueva con variables crudas.
# ====================================================================================
...
# CONFIG
# ------------------------------------------------------------------------------------
# IMPORTANTE: aquí va TU bloque de configuración existente (TGT, TARGET_COL, SIGN, _RD,
# OP, CMAP, NUM_VARS, ORD_VAR, VARS_DIRECTAS, SCORE_COL, SIT_COL, SCORE_BANDA, CORTES,
# USA_SCORE_FEATURE, DIRECCION_NEGOCIO, WOE_AL_ARBOL, BINS_OBJETIVO, OPTB_SAMPLE,
# MAX_DEPTH, MIN_SAMPLES_LEAF, RANDOM_STATE, SENTINEL, APETITO, FACTORES,
# TOP_N_ESTRATEGIAS, LEADS_EN_EXCEL_MAX, SUBJECT_COL, FLG_COL, VALORES_FLG, FAR_COL,
# VARS_EXCLUIR, BOLD, BORDER, fmt_target, ...).
#
# Solo se AGREGA esta constante nueva:
# ====================================================================================
UMBRAL_MISSING_IMPUTA = 0.05   # >5% de missing -> imputa con WoE del bin de MAYOR riesgo

# (si no las tienes ya definidas como estilos globales)
# BOLD = Font(bold=True)
...
def procesar_escenario(wb, nombre, df_sc, metodologia, excluir=()):
    ...
    optbs_guardados, imputados = {}, {}              # <-- para guardar el modelo
    ...
    for c in feats:
        ...
        if metodologia == "B" and c in NUM_VARS:
            ...
            if usa_woe:
                ...
                # --- imputar missings (>5%) con el WoE del bin de MAYOR riesgo ---
                #     RD  (SIGN=+1): mayor riesgo = mayor RD  promedio -> max WoE
                #     score (SIGN=-1): mayor riesgo = menor score promedio -> min WoE
                ...
                optbs_guardados[c] = optb            # se guarda el binning para scoring
            else:
    ...
    # ----- 11) Modelo del escenario (para guardar y reaplicar) -----
    modelo_sc = {
        "tree": tree,
        "feats": feats,
        "cst": dict(zip(feats, cst)),
        "optbs": optbs_guardados,
        "usa_woe": usa_woe,
        "imputa_missing": imputados,        # {var: WoE de mayor riesgo}
        "woemap": woemap,
        "leafmap": leafmap,
        "leaf_val": leaf_val,
        "regla_txt": regla_txt,
...
def construir_workbook(df, metodologia, ruta, excluir=()):
    ...
    for nombre, sub in construir_escenarios(df):
        leads, texto, modelo_sc = procesar_escenario(wb, nombre, sub, metodologia, excluir)
    ...
    txt_estr, estrategia_rank = hoja_estrategia(wb, leads_total, ruta)
    wb.save(ruta)
    ...
    # ----- Guardar el MODELO completo (para reaplicar a una base nueva) -----
    bundle = {
        "escenarios": modelos,
        "estrategia_rank": estrategia_rank,
        "config": {
            ...
            "SCORE_COL": SCORE_COL, "SIT_COL": SIT_COL, "SCORE_BANDA": SCORE_BANDA,
            "USA_SCORE_FEATURE": USA_SCORE_FEATURE, "CORTES": CORTES,
...
def main(df=None):
    ...
    df = base_estrategia_sinnulospd
    print(f"Base: {len(df):,} filas, {df.shape[1]} columnas  |  OBJETIVO = {TGT}  (target = {TARGET_COL})")

    construir_workbook(df, "A", f"arbol_solo_{TGT}_V2.xlsx")
    construir_workbook(df, "B", f"optbinning_arbol_{TGT}_V2.xlsx")
    print(f"Listo: arbol_solo_{TGT}.xlsx y optbinning_arbol_{TGT}_V2.xlsx")

    excluir = [c for c in VARS_EXCLUIR if c in df.columns]
    if excluir:
        construir_workbook(df, "A", f"arbol_solo_{TGT}_sinalgunasvariables_V2.xlsx", excluir)
        construir_workbook(df, "B", f"optbinning_arbol_{TGT}_sinalgunasvariables_V2.xlsx", excluir)
