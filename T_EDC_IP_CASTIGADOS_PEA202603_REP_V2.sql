CREATE TABLE disc_model_owner.T_EDC_IP_CASTIGADOS_PEA202603_REP_V2
WITH (format = 'PARQUET', parquet_compression = 'SNAPPY')
AS
-- =========================================================================
-- 1) BASE CASTIGADOS PEA (antes T_EDC_IP_CASTIGADOS_PEA202603_REP)
-- =========================================================================
WITH PEA AS (
    select a.* from awsdatacatalog.e_perm_aws.ds_rtd_rskvol_indicadores_pea a
    WHERE p_fecinformacion = '20260309'                                  -- <<< 1
)
...
, df_rcc_castigos_tot as (
    ...
    WHERE SUBSTRING(cod_cuenta_rcc,1,4) IN ('8113','8123','8133')
      ...
      AND date_parse(concat(codmes,'01'), '%Y%m%d')
          BETWEEN date_add('month', -23, date_parse('20260101','%Y%m%d'))-- <<< 3
              AND date_add('month',   0, date_parse('20260101','%Y%m%d'))-- <<< 4
)
...
, df_castigo_u24M_ibk as (
    ...
    WHERE cod_instit_financiera = '00002'
      ...
      AND condicion > 30
      AND date_parse(concat(codmes,'01'), '%Y%m%d')
          BETWEEN date_add('month', -23, date_parse('20260101','%Y%m%d'))-- <<< 6
              AND date_add('month',  -1, date_parse('20260101','%Y%m%d'))-- <<< 7
)
...
, df_castigo_u24M_noibk as (
    SELECT key_value, tip_doc, codmes u_codmes,
        date_diff('month',
            ...
            CAST(date_parse('202601','%Y%m') AS date)                    -- <<< 9
        ) AS ant_ureporte,
    ...
    FROM (
        ...
        WHERE cod_instit_financiera <> '00002'
          ...
          AND condicion > 30
          AND date_parse(concat(codmes,'01'), '%Y%m%d')
              BETWEEN date_add('month', -23, date_parse('20260101','%Y%m%d')) -- <<< 10
                  AND date_add('month',  -1, date_parse('20260101','%Y%m%d')) -- <<< 11
...
, base_pea AS (
    select
        a.SUBJECT_ID
        ,split_part(a.SUBJECT_ID,'-',1) AS COD_TIP_DOC          -- llave nueva para los joins
...
-- =========================================================================
-- 3) FEATURE INCA (party_id -> key_value vía base_cliente, con desfase 1 mes)
-- =========================================================================
, inca_rf AS (
    SELECT
        bc.tip_doc   AS cod_tip_doc,
        bc.key_value AS key_value,
        -- process_date - 1 mes  = cod_mes  (tu fórmula). Cambia -1 por 1 si querías el mes anterior del inca.
        date_format(date_add('month', -1,
            date_parse(CAST(inca.process_date AS VARCHAR), '%Y%m')), '%Y%m') AS cod_mes_join,
    ...
    GROUP BY
        ...
        date_format(date_add('month', -1,
            date_parse(CAST(inca.process_date AS VARCHAR), '%Y%m')), '%Y%m')
)
-- =========================================================================
-- 4) SELECT FINAL: base + pivot + score (maestro) + segmentacion_gdp_v2 + inca + origenapp
-- =========================================================================
SELECT
    base.*,
    -- Score ahora proviene de t_rsk_maestro_score (merge por periodo / key_value / tipdoc)
    ms.prob_modelo,
    ms.puntuacion,
    ms.prob_modelo_cal,
    ms.puntuacion_cal,
    ...
    ms.segmentacion_gdp,
    ms.nom_modelo,
    -- segmentacion_gdp_v2 desde t_rsk_segmentacion_gdp (merge por tipdoc / key_value / codmes)
...
LEFT JOIN awsdatacatalog.e_perm_aws.t_rsk_maestro_score ms
       ON base.codmes_pea  = ms.periodo
      AND base.key_value   = ms.key_value
      AND base.cod_tip_doc = ms.tipdoc
      AND ms.nom_modelo    = 'Score Originación TC No Bank 2024'   -- <<< filtro en ON (conserva LEFT JOIN)
LEFT JOIN awsdatacatalog.e_perm_aws.t_rsk_segmentacion_gdp sg
       ON sg.codmes      = base.codmes_pea
...
LEFT JOIN inca_rf ir
      ...
      AND ir.cod_mes_join = base.codmes_pea          -- desfase 1 mes
