# -*- coding: utf-8 -*-
"""Generador reutilizable: de una tabla 'CUADRO DEL ÁRBOL COMPLETO' (TSV) -> HTML
   con 2 pestañas (segmentos + optimizador) y (árbol ramificado interactivo)."""
INF = float("inf")
...
DISP = {
    ...
    "nro_entidades_castigo": "nro_ent_cast", "nro_entidades_castigo_vida": "nro_ent_cast_vida",
    "max_dias_mora_castigo": "dias_mora", "meses_desde_ultimo_castigo": "meses_ult_cast",
    "meses_desde_primer_castigo": "meses_1er_cast", "edad_num": "edad", "rk_ing_num": "ingreso(rk)",
    "saldo_fdp_tot_txs_u3m": "txs_fdp_u3m", "saldo_fdp_tot_txs_u6m": "txs_fdp_u6m",
    ...
    "saldo_prom_tot_pasivo_u6m": "pasivo_u6m", "saldo_prom_tot_pasivo_max_u6m": "pasivo_max_u6m",
    "saldo_pasivo_componentes_u3m": "pasivo_comp", "prom_saldo_pasivo_u4m": "pasivo_u4m",
    "ratio_txs_pasivo_u3m": "ratio_txs_pasivo", "puntuacion_cal_cat": "cat_score",
...
def parse_tabla(raw):
    ...
    for ln in lines[1:]:
        c = ln.split("\t")
        n = int(num(c[nv])); sc = float(c[nv + 2]); q = c[nv + 3].strip()
        conds, iv = {}, {}
        for i, v in enumerate(varcols):
            val = c[i].strip()
            if val and val != "(todos)":
                conds[v] = val; iv[v] = parse(v, val)
        leaves.append({"n": n, "sc": sc, "q": q, "iv": iv, "conds": conds})
    return leaves

TPL = r"""<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Segmentación de riesgo — @@NOMBRE@@</title>
<style>
...
<header>
  <h1>🌳 Segmentación de riesgo — @@NOMBRE@@</h1>
  <div class="s">@@TOTALF@@ clientes · @@NLEAVES@@ segmentos · objetivo = score · de menor a mayor riesgo</div>
  <div class="tabs">
...
<section class="view on" id="v0">
  <div class="opt">
    ...
    <div class="h">Mueve o escribe el apetito (score mínimo). Un segmento entra si su score promedio ≥ apetito. Score promedio = proxy de riesgo.</div>
    <div class="slider">
      <span>apetito ≥</span>
      <input type="range" id="thr" min="@@SMIN@@" max="@@SMAX@@" step="1" value="@@DEF@@"
             oninput="document.getElementById('thrn').value=this.value;upd()">
      <input type="number" id="thrn" value="@@DEF@@" style="width:90px;font-size:20px;font-weight:800;color:#007a72;border:2px solid #bfe0da;border-radius:8px;padding:4px 8px;text-align:center"
    ...
    <div class="kpis">
      ...
      <div class="kc"><div class="t" id="kmt">Si bajo −5%</div><div class="v" id="kmn">–</div><div class="d" id="kms"></div></div>
      <div class="kc"><div class="t" id="kpt">Si subo +5%</div><div class="v" id="kpn">–</div><div class="d" id="kps"></div></div>
    </div>
...
<script>
 ...
 function pool(thr){let n=0,sw=0; for(const[s,c]of SEG) if(s>=thr){n+=c;sw+=s*c;} return {n:n,pct:100*n/TOTAL,avg:n?sw/n:0};}
 ...
 function hlt(t){t=t.trim().toLowerCase();
   ...
   if(t)setAll(true);}
 function chart(thr){
   const L=60,R=950,T=20,B=260, xs=s=>L+(s-SMIN)/(SMAX-SMIN)*(R-L), ys=n=>B-(n/TOTAL)*(B-T);
   let pts=[],step=(SMAX-SMIN)/120;
   for(let s=SMIN;s<=SMAX+1e-6;s+=step) pts.push([xs(s),ys(pool(s).n)]);
   const poly=pts.map(p=>p[0].toFixed(1)+','+p[1].toFixed(1)).join(' ');
   const area=xs(SMIN).toFixed(1)+',260 '+poly+' '+xs(SMAX).toFixed(1)+',260';
   const mx=xs(thr); let g='';
   for(let k=0;k<=4;k++){const v=TOTAL*k/4,y=ys(v);
   ...
   for(let k=0;k<=4;k++){const v=SMIN+(SMAX-SMIN)*k/4,x=xs(v);
 ...
 function upd(){
   const thr=+document.getElementById('thrn').value||0, p=(+document.getElementById('pct').value||0)/100;
   const tlo=thr*(1-p), thi=thr*(1+p), b=pool(thr), lo=pool(tlo), hi=pool(thi);
   ...
   document.getElementById('kmt').textContent=`Si bajo −${pp}% (≥ ${tlo.toFixed(0)})`;
   document.getElementById('kpt').textContent=`Si subo +${pp}% (≥ ${thi.toFixed(0)})`;
...
def _parts(leaves, apetito_def=None):
    """Devuelve las piezas (rows, tree, seg, smin, smax, total, deff, nleaves) de un escenario."""
    TOTAL = sum(l["n"] for l in leaves)
    SMIN = min(l["sc"] for l in leaves); SMAX = max(l["sc"] for l in leaves)

    def color(sc):
        t = (sc - SMIN) / (SMAX - SMIN) if SMAX > SMIN else 0.5
        t = max(0.0, min(1.0, t))
        if t < 0.5:
            f = t / 0.5; r = 215 + 40 * f; g = 48 + 207 * f; b = 39 + 152 * f
        else:
            f = (t - 0.5) / 0.5; r = 255 - 229 * f; g = 255 - 103 * f; b = 191 - 111 * f
    ...
    grp = {}
    for l in leaves:
        grp.setdefault(l["conds"].get("puntuacion_cal_cat", "(todos)"), []).append(l)
    ...
    deff = apetito_def if apetito_def is not None else (730 if SMIN <= 730 <= SMAX else round((SMIN + SMAX) / 2))
    return {"total": TOTAL, "smin": int(SMIN), "smax": int(SMAX) + 1, "deff": deff,
            "nleaves": len(leaves), "rows": rows, "tree": TREE, "seg": SEG}
  ...
  <section class="view on" id="v0{k}">
    <div class="opt">
      ...
      <div class="slider"><span>apetito ≥</span>
        ...
        <input type="number" id="thrn{k}" value="{p['deff']}" style="width:90px;font-size:20px;font-weight:800;color:#007a72;border:2px solid #bfe0da;border-radius:8px;padding:4px 8px;text-align:center"
      ...
      <div class="kpis">
        ...
        <div class="kc"><div class="t" id="kmt{k}">Si bajo −5%</div><div class="v" id="kmn{k}">–</div><div class="d" id="kms{k}"></div></div>
        <div class="kc"><div class="t" id="kpt{k}">Si subo +5%</div><div class="v" id="kpn{k}">–</div><div class="d" id="kps{k}"></div></div>
      </div>
      <svg id="chart{k}" viewBox="0 0 1000 300" preserveAspectRatio="none"></svg>
    </div>
    <div class="bar"><span class="legend">menor riesgo <span class="grad"></span> mayor riesgo</span>
      <input class="f" placeholder="filtrar… (ej. edad, mora)" oninput="filt({k},this.value)"></div>
    <table><thead><tr><th>#</th><th>score</th><th>Q</th><th>n</th><th>%</th><th>Regla del segmento</th></tr></thead>
    <tbody>{p['rows']}</tbody></table>
  </section>
  <section class="view" id="v1{k}">
    <div class="bar"><span class="legend">raíz = <b>cat_score</b> (reglas reales); dentro se ramifican las demás variables</span>
...
<script>
 ...
 function upd(k){
   ...
   document.getElementById('k0s'+k).textContent=b.pct.toFixed(1)+'% base · score prom '+b.avg.toFixed(0);
   const dlo=lo.n-b.n, dhi=hi.n-b.n, pp=p*100;
   ...
   document.getElementById('kms'+k).innerHTML=`<span class="${dlo>=0?'up':'dn'}">${dlo>=0?'+':''}${fmtN(dlo)} leads</span> · score prom ${lo.avg.toFixed(0)}`;
...
def build_html(nombre, raw, salida, apetito_def=None):
    leaves = parse_tabla(raw)
    TOTAL = sum(l["n"] for l in leaves)
    SMIN = min(l["sc"] for l in leaves); SMAX = max(l["sc"] for l in leaves)
    ...
    SEG = "[" + ",".join(f"[{l['sc']:.2f},{l['n']}]" for l in leaves) + "]"
    deff = apetito_def if apetito_def is not None else (730 if SMIN <= 730 <= SMAX else round((SMIN + SMAX) / 2))
    html = (TPL.replace("@@NOMBRE@@", nombre).replace("@@TOTALF@@", f"{TOTAL:,}")
            .replace("@@NLEAVES@@", str(len(leaves))).replace("@@SMIN@@", str(int(SMIN)))
            .replace("@@SMAX@@", str(int(SMAX) + 1)).replace("@@DEF@@", str(deff))
            .replace("@@ROWS@@", rows).replace("@@TREE@@", TREE)
            .replace("@@SEG@@", SEG).replace("@@TOTAL@@", str(TOTAL)))
    with open(salida, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"OK {salida} | {len(leaves)} hojas | n={TOTAL:,} | score {SMIN:.0f}-{SMAX:.0f}")
    return leaves
